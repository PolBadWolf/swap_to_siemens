...
#define LCD_EI_DDR		bit_is_byte(DDRA).bit4
#define LCD_EI			bit_is_byte(PORTA).bit4

#define LCD_RS_DDR		bit_is_byte(DDRA).bit5
#define LCD_RS			bit_is_byte(PORTA).bit5
#else
//--
...
#define LCD_EI_DDR		bit_is_byte(DDRB).bit5
#define LCD_EI			bit_is_byte(PORTB).bit5

#define LCD_RS_DDR		bit_is_byte(DDRB).bit4
#define LCD_RS			bit_is_byte(PORTB).bit4
#else
...
void Lcd_hard::putChar_X(uint8_t *pos, char sim) {
	((Lcd *)Lcd_hard::obj)->PutChar(pos, sim);
}
...
int putCharX(char c, FILE *stream)
{
	...
	p = &(Lcd_hard::obj->pos);
// 	thisObj->PutChar(p, c);
	Lcd_hard::putChar_X(p, c);
	return 1;
}
FILE* Lcd_hard::Init(uint8_t stolbcov)
{
	// настройка портов
	LCD_D_DDR = 0x0f;
	LCD_D = 0x0f;
...
void Lcd_hard::Interrupt_static() {
	obj->Interrupt();
}
